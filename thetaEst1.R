...
source("eapEst1.R")
#Run function
thetaEst1 = function (it, x, model = NULL, D = 1, method = "BM", priorDist = "norm", 
          priorPar = c(0, 1), range = c(-4, 4), parInt = c(-4, 4, 33), 
          constantPatt = NULL, current.th = 0, bRange = c(-2, 2)) 
{
  eapEst <- eapEst1 #Call the updated function
  ...
  if (!constantPattern(x) | !is.null(model) | is.null(constantPatt)) 
    ...
    if (sum(constantPatt == c("BM", "EAP", "WL")) == 
        1) 
      METHOD <- constantPatt
    else {
      if (sum(x) == 0) 
  ...
  if (!is.null(METHOD)) {
    if (METHOD == "EAP") 
      ...
      if (is.null(model)) {
        ...
        f0 <- function(th) {
          if (met == 2) 
            res <- switch(pd, `1` = (priorPar[1] - 
                                       th)/priorPar[2]^2, `2` = 0, `3` = sum(Ii(th, 
                                                                                it, model = model, D = D)$dIi)/(2 * sum(Ii(th, 
                                                                                                                           it, model = model, D = D)$Ii)))
          else res <- switch(met, `1` = 0, `2` = 0, 
                             `3` = sum(Ji(th, it, model = model, 
                                          D = D)$Ji)/(2 * sum(Ii(th, it, model = model, 
                                                                 D = D)$Ii)))
          return(res)
        }
