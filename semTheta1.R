#Call the updated eapSem
source("eapSem1.R")
#Run the function
semTheta1 = function (thEst, it, x = NULL, model = NULL, D = 1, method = "BM", 
          priorDist = "norm", priorPar = c(0, 1), parInt = c(-4, 
                                                             4, 33), constantPatt = NULL) 
{
  eapSem <- eapSem1
  ...
  if (!is.null(METHOD)) {
    if (method == "EAP") {
      ...
      if (is.null(model)) {
        ...
        res <- -dr0(thEst, it, D = D, method = method, 
