Pi1 = function (th, it, model = NULL, D = 1) 
{
  #it <- rbind(it)
  if (is.null(model)) {
    a <- it[, 1]
...
#microbenchmark(aPi = Pi(th=th, it=it1[1:12,]), unit = 'ms')
#microbenchmark(aPi1 = Pi1(th=th, it=it1[1:12,]), unit = 'ms')
